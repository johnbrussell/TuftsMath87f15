function [profit, lost_cust, spaced_out, perished, targets, sold] = perishablenonfifo(demand, storage_space, wait_time, lifespan, markup, sell)
    ...
    for i=1:num_days
        ...
        for k=1:3
            if customers(k) >= stock(k)
                ...
                while customers(k) > 0
                    while false
                        rage = round(rand * lifespan(k) + .5);
                        if(age(k, rage) > 0)
                            age(k, rage) = age(k, rage) - 1;
                            break;
                        end
                    end
